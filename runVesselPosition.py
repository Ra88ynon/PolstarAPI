        ...
        for item in positionData['objects']:
                ...
                if item.get('last_position') is not None:
                        ...
                        if len(item.get('last_position').get('position_info').get('cyclones')) > 0:
                                vesselPosition['cyclone_name'] = item.get('last_position').get('position_info').get('cyclones')[0].get('track')[0].get('name')
                                vesselPosition['cyclone_type'] = item.get('last_position').get('position_info').get('cyclones')[0].get('track')[0].get('storm_type')
                                vesselPosition['cyclone_distance'] = item.get('last_position').get('position_info').get('cyclones')[0].get('track')[0].get('distance')
                                vesselPosition['cyclone_latitude'] = item.get('last_position').get('position_info').get('cyclones')[0].get('track')[0].get('latitude')
                                vesselPosition['cyclone_longitude'] = item.get('last_position').get('position_info').get('cyclones')[0].get('track')[0].get('longitude')
                                vesselPosition['cyclone_speed'] = item.get('last_position').get('position_info').get('cyclones')[0].get('track')[0].get('speed')
                        else :
                ...
                logging.debug('Inserting vessel position for : ' + vesselPosition['imo_number'] + ' latitude : ' +
                              vesselPosition['latitude'] + ' longitude :' + vesselPosition['longitude'] + ' cyclone_name :' + vesselPosition['longitude'])
